...
![](https://ws1.sinaimg.cn/large/8747d788gy1fylqth0oymj21im0thn0a.jpg)
...
前提：一个模块中可能使用多个key，一个类代表一个模块，一个实例代表模块下的子模块key；使用Enum不能复用父类的getPrefix()，要每个Enum都要写一次。。
...
2次md5，客户端一次（防止网络被截取包，因为http是不加密的），服务端一次（防止数据库泄露后，明文密码）

* 以上的问题：客户端的代码是暴露的，MD5的处理过程完全透明，没啥意义

#### 2.3 jsr303参数校验，自定义注解
...
#### 2.4 分布式session，redis，延长cookie时间 
